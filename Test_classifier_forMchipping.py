...
from cfg_forMchipping import ConfigData, HypScratch
...
def test(opt):
    opt.img_folder = r'D:\BackendAOI\A_Training\4KM5-2801-TR1C_(V1)\Bin2-chipping\2_testdata\data\add_20231219\LSG'
    opt.weights = r'D:\BackendAOI\Python\cnn\save_model\4KM5-2801-TR1C_(V2)\4class\best.pt'
    ...
    os.makedirs(opt.img_folder+'\\4class_output_report', exist_ok=True)
    txt_file = open(opt.img_folder+'\\4class_output_report\\results.txt','w')
    os.makedirs(opt.img_folder+'\\4class_output_report\\pass_output', exist_ok=True)
    os.makedirs(opt.img_folder+'\\4class_output_report\\fail_output', exist_ok=True)
    ...
    with torch.no_grad():
        for index, file in enumerate(files):
            ...
            if f'{pred}' == '0':
                    ...
                    cv2.imwrite(opt.img_folder+'/4class_output_report/pass_output/'+'ChippingPass'+(str(outputs).split(", ")[0]).split('([[')[1]+"_"+(str(outputs).split(", ")[1]).split(']]')[0]+".jpg",pass_img)
            if f'{pred}' == '1':
                    pass_img = cv2.imread(f'{file}')
                    cv2.imwrite(opt.img_folder+'/4class_output_report/pass_output/'+'FoundryLayout'+(str(outputs).split(", ")[0]).split('([[')[1]+"_"+(str(outputs).split(", ")[1]).split(']]')[0]+".jpg",pass_img)
            if f'{pred}' == '2':
                    pass_img = cv2.imread(f'{file}')
                    cv2.imwrite(opt.img_folder+'/4class_output_report/pass_output/'+'LSG'+(str(outputs).split(", ")[0]).split('([[')[1]+"_"+(str(outputs).split(", ")[1]).split(']]')[0]+".jpg",pass_img)
            if f'{pred}' == '3':
                    fail_img = cv2.imread(f'{file}')
                    cv2.imwrite(opt.img_folder+'/4class_output_report/fail_output/'+'Ng'+(str(outputs).split(", ")[0]).split('([[')[1]+"_"+(str(outputs).split(", ")[1]).split(']]')[0]+".jpg",fail_img)
