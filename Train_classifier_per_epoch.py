...
def train(opt):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            # save per 10 epoch .pt model
            if epoch/10 == int(save_epoch_num):
                save_epoch = save_epoch_num * 10
                epoch_model_pt = copy.deepcopy(model.state_dict())
                save_path = f'{output_folder}\\'+str(save_epoch)+'_epoch.pt'
                torch.save(epoch_model_pt, save_path)
                save_epoch_num = save_epoch_num+1
            # save best f1_score
